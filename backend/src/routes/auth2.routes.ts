  ...
  try {
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p style="color:#666;font-size:12px;">该链接24小时内有效。</p>
      </div>`;
    const text = `请打开以下链接完成邮箱验证（24小时内有效）：\n${verifyUrl}`;
...
router.post('/login', async (req, res) => {
  ...
  if (!ok) throw new AppError('邮箱或密码错误', 401, 'INVALID_CREDENTIALS');
...
router.get('/verify', async (req, res) => {
  ...
  if (!record) throw new AppError('无效的验证令牌', 400, 'TOKEN_INVALID');
  if (record.usedAt) throw new AppError('验证令牌已使用', 400, 'TOKEN_USED');
  if (record.expiresAt.getTime() < Date.now()) throw new AppError('验证令牌已过期', 400, 'TOKEN_EXPIRED');
...
router.post('/resend-verification', async (req, res) => {
  ...
  const user = await prisma.user.findUnique({ where: { email } });
  if (!user) return res.json({ success: true, message: '如果邮箱存在，将发送验证邮件' });
  if (user.emailVerified) return res.json({ success: true, message: '邮箱已完成验证' });
  const token = crypto.randomBytes(32).toString('hex');
  const expiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000);
  await prisma.emailVerificationToken.create({ data: { userId: user.id, token, expiresAt } });
  const frontendURL = process.env.FRONTEND_URL || 'http://localhost:5173';
  ...
  const html = `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">`+
               ...
               `<p style="color:#666;font-size:12px;">该链接24小时内有效。</p></div>`;
  const text = `请打开以下链接完成邮箱验证（24小时内有效）：\n${verifyUrl}`;
  await SendGridService.send({ to: email, subject, html, text });
